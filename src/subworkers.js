      ...
      self.Worker = function(path){
        ...
        self.addEventListener("message", function(e){
          if (e.data._from === that.id){
            ...
            newEvent.initMessageEvent("message", false, false, 
e.data.message, that, "", null, []);
        ...
        var absPath = location.substring(0, location.lastIndexOf('/')) + 
slashedPath;
        self.postMessage({
          _subworker: true,
          cmd: 'newWorker',
...
event.data.transfer);
  ...
  Worker = function(path){
    if (this.constructor !== Worker){
      throw new TypeError("Failed to construct 'Worker': Please use the 
